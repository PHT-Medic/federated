...
def test_create_discovery(proposal_id):
    response = client.post(
        f"/api/proposal", json={"id": proposal_id, "name": "Discovery test proposal"}
    )
    print(response.json())
    response = client.post(
...
def test_update_discovery():
    proposal_id, discovery_id = create_proposal_and_discovery()
    response = client.put(
        f"/api/proposal/{proposal_id}/discoveries/{discovery_id}",
    ...
    # 404 if proposal does not exist
    response = client.put(
        f"/api/proposal/{uuid4()}/discoveries/{discovery_id}",
        json={"query": {"hello": "world2"}},
    )
    assert response.status_code == 404, response.text
    ...
    # 404 if discovery does not exist
    response = client.put(
        f"/api/proposal/{proposal_id}/discoveries/{99999}",
        json={"query": {"hello": "world2"}},
    )
    assert response.status_code == 404, response.text
...
def test_discovery_get_selected():
    ...
    assert response.status_code == 200, response.text
    response = client.get(
        f"/api/proposal/{proposal_id}/discoveries/{discovery_id}/summary?features={SELECTED_FEATURES}"
    )
    assert response.status_code == 200, response.text
