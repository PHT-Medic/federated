    ...
    for i, user_key in enumerate(keys):
        ...
        for j, recipient in enumerate(keys):
            if i == j:
                pass
            if i > j:
                mask += generate_shared_mask(
                    private_key=user_key.cipher_key,
                    public_key=recipient.cipher_key_public,
                    n_items=100,
                )
            if i < j:
                mask -= generate_shared_mask(
